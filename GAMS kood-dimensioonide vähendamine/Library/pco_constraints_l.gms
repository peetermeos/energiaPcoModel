...
* Storage constraints
  ...
  v_min_storage_t(time_t, s_t)            "Ladude miinimumi piirang (t)"
...
Parameter fix_st(storage, k, feedstock);

* fix_st is recalculated in calendar formulation code
* either calendar_day or calendar_month depending on
* time resolution required for the model

$GDXin _gams_net_gdb0.gdx
$load fix_st=fix_ladu
$GDXin
...
** Taaniel Uleksin                                                             *
********************************************************************************
v_min_storage_t(time_t, s_t)$time_t_s(time_t)..
   sum((feedstock, k), storage_t(time_t, s_t, k, feedstock))
   =g=
   min_storage(s_t)
;
...
* Storage tomorrow
   storage_k(time_t + 1, s_k, k ,feedstock)
   =e=
* Storage today
...
********************************************************************************
**                                                                             *
** Last day's feedstock cannot enter storage (but can to to logistics)         *
**                                                                             *
** Taaniel Uleksin                                                             *
********************************************************************************
v_k_storage2(time_t, s_k, k, feedstock)
                     $(time_t_s(time_t)
                   and ord(time_t) eq card(time_t))..
...
********************************************************************************
**                                                                             *
** Last day's feedstock outflow cannot violate minimum storage level constraint*
**                                                                             *
** Taaniel Uleksin                                                             *
********************************************************************************
v_k_storage3(time_t, s_k)
                     $(time_t_s(time_t)
                   and ord(time_t) eq card(time_t))..

  sum((k, feedstock), sum((route, l)$(route_endpoint(route, k, l)
               and mine_storage(k, s_k)
               and fs_k(k, feedstock)),
        storage_to_logs(time_t, s_k, route, feedstock)
        ))
=l=
  sum((k, feedstock), storage_k(time_t, s_k, k, feedstock))
  -
  min_storage(s_k)
;
...
********************************************************************************
**                                                                             *
** Last day's feedstock cannot enter storage (but can go to production)        *
**                                                                             *
** Taaniel Uleksin                                                             *
********************************************************************************
v_t_storage2(time_t, s_t, k, feedstock)
                        $(time_t_s(time_t)
                     and ord(time_t) eq card(time_t))..
...
=l=
  sum((k, feedstock), storage_t(time_t, s_t, k, feedstock)
                                                         $(fs_k(k, feedstock)))
  -
  min_storage(s_t)
;
