...
** Peeter Meos                                                                 *
********************************************************************************
z_emission.up(time_t, slot, k, "Uttegaas", t_el, para_lk)
                                                 $(cv("Uttegaas", k, "MWh") > 0)
     = sum((year, month)$y_m_t, t_rg(t_el, year, month))
                              * cv("Uttegaas", k, "MWh");
...
v_min_cv(time_t, t)$(time_t_s(time_t) and sum((year, month)
 ...
 sum((k, feedstock),
     to_production(time_t, k, feedstock, t) * cv(feedstock, k, "MWh"))
=g=
...
* For monthly resolution, the following needs to be switched off
  (
    ...
    - t_stop(time_t, t_el)     * round(num_days(clean_span, time_t, t_el))
   )
;
