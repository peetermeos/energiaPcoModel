...
Variable
   ...
   t_production(sim, year, month, t, product)      "Production amount"
;
...
*fs_vc_s(sim, k, feedstock, year)$(sum((month, time_t)$y_m_t, 1) > 0
*                                and fs_vc(k, feedstock, year) > 0
*                                and not (sameas(k, "Estonia") and sameas(feedstock, "Kaevis"))
*                                and (sameas(k, "Estonia") or sameas(k, "Narva1") or sameas(k, "Viivikonna") )
*                                 )
*  = fs_vc(k, feedstock, year) + h_run(sim) * cv(feedstock, k, "MWh");
