...
*Toodangud
  ...
  t_production_year(sim, year, t, product)                             "Tootmisüksuse toodang aastas (MWh(el), MWh(sj), m3 uttegaasi või t(õli))"
...
*Kulud/Kasumid
  ...
  t_varcost_fs_slot(sim, time_t, slot, t, k, feedstock, product, vc)             "Tootmisüksuse muutuvkulud komponentide kaupa kütustele slotis (EUR)"
  t_varcost_fs_day(sim, time_t, t, k, feedstock, product, vc)                    "Tootmisüksuse muutuvkulud komponentide kaupa kütustele päevas (EUR)"
  t_varcost_fs_month(sim, year, quarter, month, t, k, feedstock, product, vc)       "Tootmisüksuse muutuvkulud komponentide kaupa kütustele kuus (EUR)"
  t_varcost_fs_year(sim, year, t, k, feedstock, product, vc)                     "Tootmisüksuse muutuvkulud komponentide kaupa kütustele aasta (EUR)"
  t_varcost_fs_perunit_year(sim, year, t, k, feedstock, product, vc)             "Tootmisüksuse muutuvkulud komponentide kaupa kütustele aasta (EUR/MWh(el), EUR/MWh(sj) või EUR/t(õli))"
...
Parameter log_vc_fs(t, k, feedstock, year);

log_vc_fs(t, k, feedstock, year)$fs_k(k, feedstock) =
 smin((route, l)$(route_endpoint(route, k, l) and t_dp_prod(l, t)), log_vc(route, year))
;
log_vc_fs(t, k, feedstock, year)$(log_vc_fs(t, k, feedstock, year)$fs_k(k, feedstock) > M) = 0;

t_varcost_fs_slot.l(sim, time_t, slot, t, k, feedstock, product, vc)$(max_ratio(k, feedstock, t) > 0)
=
*Kulud heitmetele
...
*Logistika
  +
  (
           sum((year, month)$y_m_t, log_vc_fs(t, k, feedstock, year))
         * fuel_consumption_slot(sim, time_t, slot, t, product, k, feedstock, "t")
  )$(sameas(vc, "logist") and fuel_consumption_slot(sim, time_t, slot, t, product, k, feedstock, "t") > 0)
*Kütuse kulu
  +
  (
         (
...
t_varcost_fs_slot.l(sim, time_t, slot, t, k, feedstock, product, "kokku")$(max_ratio(k, feedstock, t) > 0)
=
  sum(vc$(not sameas(vc, "kokku")), t_varcost_fs_slot.l(sim, time_t, slot, t, k, feedstock, product, vc))
;

t_varcost_fs_day.l(sim, time_t, t, k, feedstock, product, vc)$(max_ratio(k, feedstock, t) > 0)
=
  sum(slot, t_varcost_fs_slot.l(sim, time_t, slot, t, k, feedstock, product, vc))
;

t_varcost_fs_month.l(sim, year, quarter, month, t, k, feedstock, product, vc)$(max_ratio(k, feedstock, t) > 0 and q_months(quarter, month))
=
  sum(time_t$y_m_t, t_varcost_fs_day.l(sim, time_t, t, k, feedstock, product, vc))
;

t_varcost_fs_year.l(sim, year, t, k, feedstock, product, vc)$(max_ratio(k, feedstock, t) > 0)
=
  sum((quarter, month)$q_months(quarter, month), t_varcost_fs_month.l(sim, year, quarter, month, t, k, feedstock, product, vc))
;
...
=
  t_varcost_fs_year.l(sim, year, t, k, feedstock, product, vc)
  /
  sum((quarter, month)$q_months(quarter, month), t_production_month.l(sim, year, quarter, month, t, product))
;
...
*Logistika
  +
  sum((k, feedstock)$(fs_k(k, feedstock) and fuel_consumption_slot(sim, time_t, slot, t, product, k, feedstock, "t") > 0),
*           smin((year, month, route, l)$(y_m_t and route_endpoint(route, k, l) and t_dp_prod(l, t)), log_vc(route, year))
           sum((year, month)$y_m_t, log_vc_fs(t, k, feedstock, year))
         * fuel_consumption_slot(sim, time_t, slot, t, product, k, feedstock, "t")
  )$sameas(vc, "logist")
*Kütuse kulu
...
=
  ...
  -
  t_varcost_fs_slot.l(sim, time_t, slot, t, k, feedstock, product, "kokku")
;
