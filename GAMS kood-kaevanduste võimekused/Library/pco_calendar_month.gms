...
$ife "%end_year% > %beg_year%" $evalglobal last_t (12 - %beg_month% + 1) + %end_month% + (%end_year% - %beg_year% - 1) * 12
...
$macro days_in_month_m(year, month) 1
...
$macro days_in_t(time_t) (sum((year, month)$y_m_t, days_in_month(month)              \
                           + gleap(jdate(%beg_year% + ord(year) - 1, ord(month), 1))$(ord(month) = 2)))

$macro disc_periods 12
