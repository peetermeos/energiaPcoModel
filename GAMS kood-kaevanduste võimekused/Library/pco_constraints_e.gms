...
Equations
  ...
  v_hh_emissions(time_t, slot, t_el, em)        "Emission concentration constraints"
...
v_stack_hours(t_stack)$(hour_limit(t_stack) > 0)..
     sum((time_t, slot)$(time_t_s(time_t)),
        ...
        * smax(t$(t_unit_stack(t_stack, t) and slot_length_orig(time_t, slot, t) >0), slot_length_orig(time_t, slot, t))
    )
...
** Peeter Meos                                                                 *
********************************************************************************
v_hh_emissions(time_t, slot, t_el, em)$(time_t_s(time_t)
 and sum((year, month)$y_m_t, hh_limit(em, year, t_el)) > 0
 and sum((k, feedstock, para_lk), hh_coef(em, t_el, k, feedstock, para_lk)) > 0
                          and sum(t_stack$t_unit_stack(t_stack, t_el), 1) = 0)..


   sum((k, feedstock, para_lk),
     hh_coef(em, t_el, k, feedstock, para_lk)                // [t/m3]
     * (1000 * 1000)                                         // [mg/t]
     * sum((k_level, l_level),                               // [%]
          lambda_e(time_t, slot, t_el, k, feedstock, k_level, l_level, para_lk))
   )
  =l=
    sum((year, month)$y_m_t, hh_limit(em, year, t_el))       // [mg/m3]
;
